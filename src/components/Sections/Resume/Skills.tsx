    ...
    <div ref={ref} className="flex flex-col">
      ...
      <div className="flex flex-col gap-y-3 mt-3">
        {skills.map((skill, index) => (
          <Skill key={`${skill.name}-${index}`} skill={skill} index={index} reveal={isVisible} />
        ))}
      </div>
    </div>
  );
...
export const Skill: FC<{skill: SkillType; index?: number; reveal?: boolean}> = memo(({skill, index = 0, reveal = false}) => {
